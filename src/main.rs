...
struct SidebarItemRaw {
    ...
    /// `false` for internal views/filters (Recent, Starred, Templates, Trash…) that
    /// are not launchable sub-apps. Defaults to `true` for backward compatibility.
...
#[derive(Parser, Debug)]
#[command(name = "kubuno-office", version, about = "Module documents Kubuno")]
struct Cli {
    #[arg(short, long, env = "KO_CONFIG_FILE")]
...
async fn main() -> Result<()> {
    ...
    match settings.logging.format {
        kubuno_office::config::LogFormat::Json   => subscriber.json().init(),
        kubuno_office::config::LogFormat::Pretty => subscriber.init(),
    }

    tracing::info!("Kubuno Office v{} démarrage…", env!("CARGO_PKG_VERSION"));

    // Sécurité : interdire toute exécution de processus sur l’hôte (voir kubuno-seccomp).
    kubuno_seccomp::lock_down_process_execution("office");

    // Pool PostgreSQL — search_path=office,public pour résoudre les tables sans qualification
    let opts = settings.database.connect_options()?
        .options([("search_path", "office,public")]);
    ...
    if settings.database.run_migrations {
        sqlx::query("CREATE SCHEMA IF NOT EXISTS office")
            .execute(&pool)
            .await
            .context("Création du schéma office")?;

        // Créer explicitement office._sqlx_migrations avec un nom qualifié afin que le pool
        // de migration (search_path=office,public) ne résolve pas vers public._sqlx_migrations.
        sqlx::query(
            r#"CREATE TABLE IF NOT EXISTS office._sqlx_migrations (
        ...
        .execute(&pool)
        .await
        .context("Création table office._sqlx_migrations")?;
    ...
    let state = AppState {
        ...
        sheet_hub:    Arc::new(kubuno_office::state::SpreadsheetCollabHub::new()),
        pres_hub:     Arc::new(PresentationHub::new()),
    ...
    // Heartbeat toutes les 30s
    {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let url    = format!("{}/internal/modules/office/heartbeat", settings2.core.url);
    ...
    tracing::info!("Kubuno Office démarré sur http://{addr}");
...
async fn register_with_core(http: &Client, settings: &Settings) {
    ...
    let display_name  = manifest.as_ref().map(|m| m.module.display_name.as_str()).unwrap_or("Office").to_string();
    ...
    let sidebar_items: Vec<Value> = manifest.as_ref()
        ...
        .unwrap_or_else(|| vec![
            json!({ "id": "office", "label": "Office", "icon": "FileText", "path": "/office", "position": 25, "launchable": true }),
        ]);
    ...
    for attempt in 1u32.. {
            ...
            .await
        {
            Ok(resp) if resp.status().is_success() => {
                tracing::info!("Module office enregistré auprès du core");
                return;
            }
