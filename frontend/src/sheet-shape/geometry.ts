...
// Pure geometry of the sheet's drawing SHAPES.
//
...
// be laid out in. Everything that draws a shape — the overlay on the grid, the
// insertion gallery's thumbnails, the inline text editor — reads its geometry from
// here, so a shape looks the same at every size and in every surface.
//
// Coordinate space: the shape's OWN box, from (0, 0) to (width, height). The host
// is responsible for placing that box on the grid (and for the rotation, which is
// a transform on the box, never baked into the path).
//
...
/** Les dix géométries dessinées PAR le tableur (les autres viennent du catalogue partagé). */
...
/** Une forme du catalogue partagé non dessinée ici retombe sur le rectangle : seul
 *  son CADRE DE TEXTE et sa boîte servent alors (son tracé vient de shapes/svg). */
